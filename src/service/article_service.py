...
def getPostData(post , jsonResult , cnt , src):
    ...
    # 딕셔너리 생성
    dic = {'cnt' : cnt , 'title' : title , 'description' : description , 'org_link' : org_link , 'link' : link , 'srcText' : src}
...
def get_keyword(jsonResult, srcText):
    ...
    stopwords = {'마지막', '시리즈', '프로야구', '스폰서', '코스피', '유플러스', '하반기' , '글로벌' , '신입사원' , '연구원' , '부회장' , '먹거리' , '대통령' , '플라스틱' , '결정전' , '신기록' , '굿바이' , '라이프' , '끝내기'}
...
if __name__ == "__main__":
    srcText = ['KBO' , 'KIA야구' , '삼성야구' , 'LG야구' , '두산야구' , 'KT야구' , '한화야구' , 'SSG야구' , '롯데야구' , 'NC야구' , '키움야구']
